...
GrayscaleR: .float 0.2989		# Specific Greyscale R value 
GrayscaleG: .float 0.5870		# Specific Greyscale G value
GrayscaleB: .float 0.1140		# Specific Greyscale B value 

filename: .asciiz "tesla.bmp"		# Name of our predefined file.
OutFile: .asciiz "output.bmp"		# Name of our output file.
.data    
...
B_string: .asciiz "Please enter a percentage in decimal format to increase brightness by(-100 <-> 100) :"
S_string: .asciiz "Pleaser enter a percentage in decimal format to increase saturation by (0 <-> 100)"
...
move $s5, $v0 		
la $a0, Numogbits			# 54 bmp header + (3 * Number Of Pixels);
move $a1, $v0  		
li $v0, 56	
syscall
...
beq $t0,$t1, Grayscale			# Branch to label which handles the greyscale feature
...
################ FUNCTIONALITY FOR HORIZONTAL FLIP ##################
...
loop_line:
bge $t4, 256, end_line
lw $t2, ($t0)        			# Starting swap
lw $t3, ($t1)
sw $t3, ($t0)
sw $t2, ($t1)				# Ending swap
...
################ FUNCTIONALITY FOR VERTICAL FLIP ##################
...
bge $s4, $s5, invert
lw $s2,($s0)				# Starting swap
lw $s3,($s1)				
sw $s2,($s1)				
sw $s3,($s0)				# Ending swap
...
lb $t4, 1($t6)				# Load bits
lb $t5, 2($t6)				# Load bits

sll $t3, $t3, 24			# RGB isolation process to eliminate any unnecessary bits
sll $t5, $t5, 24
sll $t4, $t4, 24
srl $t4, $t4, 24
srl $t5, $t5, 24
...
mtc1 $t3, $f3				# Moving from original registers to floating point registers (Coprocessor) 
cvt.s.w $f3,$f3				# Converting from integer to single precision.

mtc1 $t4, $f4				# Moving from original registers to floating point registers (Coprocessor) 
cvt.s.w $f4,$f4				# Converting from integer to single precision.

mtc1 $t5, $f5				# Moving from original registers to floating point registers (Coprocessor) 
cvt.s.w $f5,$f5				# Converting from integer to single precision.

mul.s $f3,$f3,$f2			# Multiplying floating register f3 by modification percentage
mul.s $f4,$f4,$f2			# Multiplying floating register f4 by modification percentage
mul.s $f5,$f5,$f2			# Multiplying floating register f5 by modification percentage

li $t9, 255				# Loading max number of color display (Range is 0 - 255)
mtc1 $t9, $f6				# Implementing logic for R
cvt.s.w $f6, $f6
c.lt.s $f3, $f6
bc1t  keepR
mov.s $f3 $f6
...
cvt.w.s $f3,$f3				# Converting floats to integer
cvt.w.s $f4,$f4		
cvt.w.s $f5,$f5
mfc1 $t3,$f3				# Moving from co processor to main registers
mfc1 $t4,$f4
mfc1 $t5,$f5
sb $t3, 0($t6)				# Storing bytes back into $t6 with the adjusted RGB values
sb $t4, 1($t6)				 
sb $t5, 2($t6)
addi $s4,$s4, 1
addi $t6, $t6, 4
...
srl $t5, $t5, 24
srl $t3, $t3, 24
					# End of RGB isolation process

################ FUNCTIONALITY FOR FINDING MAXIMUM AND MINIMUM VALUES WITHIN R, G AND B ##################	
...
l.s $f16, GrayscaleR			# Multiplying R value by greyscale R value
mul.s $f13, $f13, $f16		

l.s $f16, GrayscaleG			# Multiplying G value by greyscale G value
mul.s $f14, $f14, $f16

l.s $f16, GrayscaleB			# Multiplying G value by greyscale B value
mul.s $f15, $f15, $f16 
...
jal Flip_h
la $s0, buff        
addi $s0, $s0, 54    			# Adjsting offset to avoid haeder info
...
la $a0, OutFile    			# Opening output file
li $v0, 13
li $a1, 1
li $a2, 0
syscall
...
li $a2, 786486
syscall
bgt $v0, 0, CloseOut   			# If file not successfully open brake
la $a0,FIleErrorMsg
li $a1, 2
li $v0,55
syscall
